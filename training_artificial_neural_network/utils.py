...
def visualizeWeights(weights, save_dir, filename='weights'):
    '''visualizes the weights and saves the visualization as a png image

    Arguments:
    ----------
    weights : numpy array of size 3072 x D where D is the number of weights
    ...
    for coef, ax in zip(weights.T, axes[:num_weights].ravel()):
        coef = np.squeeze(coef).T if len(weights.shape) > 2 else coef.reshape(3, 32, 32).T
...
def visualizeDataset(images, labels, save_dir, filename='dataset', num_samples_per_class=8):
    ...
    for r in range(num_classes):
        ...
        for n in range(num_samples_per_class):
            axes[r, n].matshow(images[sample_indcs[n]].reshape(3,32,32),
                               cmap=plt.cm.gray)
            axes[r, n].set_xticks(())
            axes[r, n].set_yticks(())
